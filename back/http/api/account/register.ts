...
router.post("/register", async function (req, res) {
	...
	if (existingUserByName)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.Register_UsernameTaken,
	...
	if (existingUserByEmail)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.Register_EmailTaken,
	...
	if (!/^[a-zA-Z0-9]+$/.test(name) || name.length < 3 || name.length > 10) {
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.Register_InvalidName,
	...
	if (
		...
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.Register_InvalidPassword,
	...
	if (!mailSent) {
		...
		return res.json({
			err: true,
			c: InternalCode.ServerError.Express.Register_FailedToSendEmail,
...
router.post("/register/verify", async function (req, res) {
	if (!req.session.tmp_user)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.RegisterVerify_InvalidSession,
	...
	if (req.session.tmp_user.attemptsLeft <= 0) {
		...
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.RegisterVerify_TooManyAttempts,
	...
	if (!newUser)
		return res.json({
			err: true,
			c: InternalCode.ServerError.Express
				.RegisterVerify_FailedToRegisterUser,
