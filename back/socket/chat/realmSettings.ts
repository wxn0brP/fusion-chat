...
export default (socket: Socket) => {
	socket.onLimit(
		"realm.settings.get",
		5_000,
		async (id: Id, sections, cb: Function) => {
			try {
				if (typeof sections == "function" && !cb) {
					cb = sections;
					sections = [];
				}
				const data = await realm_settings_get(
					socket.user,
					id,
					sections,
				);
	...
	socket.onLimit(
		"realm.settings.set",
		5_000,
		async (id: Id, data, cb: Function) => {
			try {
				const event_data = await realm_settings_set(
					socket.user,
					id,
					data,
				);
	...
	socket.onLimit(
		"realm.webhook.token.get",
		5_000,
		async (realmId: Id, tokenId: Id, cb: Function) => {
			try {
				const data = await realm_webhook_token_get(
					socket.user,
					realmId,
					tokenId,
				);
