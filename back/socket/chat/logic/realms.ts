...
export async function realm_setup(
	suser: Socket_User,
	id: Id,
	...
	if (!realmMeta)
		return validE.err(
			InternalCode.UserError.Socket.RealmSetup_RealmNotFound,
		);
	...
	for (let i = 0; i < sortedCategories.length; i++) {
		...
		const allChnls = await Promise.all(
			chnlsByDb.map(async (c) => {
...
export async function realm_users_sync(
	suser: Socket_User,
	id: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_users_activity_sync(
	suser: Socket_User,
	id: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_delete(
	suser: Socket_User,
	id: Id,
	name: string,
): Promise<Socket_StandardRes> {
...
export async function realm_user_kick(
	suser: Socket_User,
	realmId: Id,
	uid: Id,
	ban: boolean = false,
...
export async function realm_user_unban(
	suser: Socket_User,
	realmId: Id,
	uid: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_emojis_sync(
	suser: Socket_User,
	realmId: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_announcement_channel_list(
	suser: Socket_User,
	realmId: Id,
): Promise<Socket_StandardRes> {
		...
		.then((channels) => {
			...
			for (const channel of channels) {
				if (subscribedChannels.some((tc) => tc.sc == channel.chid))
					continue; // no subscription to self
...
export async function realm_thread_delete(
	suser: Socket_User,
	realmId: Id,
	threadId: Id,
): Promise<Socket_StandardRes> {
	...
	if (thread.author != suser._id) {
		const permSys = new permissionSystem(realmId);
		const canAdmin = await permSys.canUserPerformAction(
			suser._id,
			Permissions.admin,
		);
		if (!canAdmin)
...
export async function realm_thread_list(
	suser: Socket_User,
	realmId: Id | null,
	channelId: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_event_create(
	suser: Socket_User,
	realmId: Id,
	req: Socket__Realms.Event__req,
): Promise<Socket_StandardRes> {
...
export async function realm_event_delete(
	suser: Socket_User,
	realmId: Id,
	eventId: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_event_list(
	suser: Socket_User,
	realmId: Id,
	len: boolean = false,
): Promise<Socket_StandardRes> {
...
export async function realm_event_join(
	suser: Socket_User,
	realmId: Id,
	eventId: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_event_leave(
	suser: Socket_User,
	realmId: Id,
	eventId: Id,
): Promise<Socket_StandardRes> {
...
export async function realm_event_get_topic(
	suser: Socket_User,
	realmId: Id,
	eventId: Id,
): Promise<Socket_StandardRes> {
	...
	if (!event)
		return validE.err(
			InternalCode.UserError.Socket.RealmEventGetTopic_NotFound,
		);

	return { err: false, res: [event.topic] };
}

export async function realm_user_role_remove(
	suser: Socket_User,
	realmId: Id,
	uid: Id,
	roleIdOrLvl: Id | number,
): Promise<Socket_StandardRes> {
	const validE = new ValidError("realm.user.role.remove");
	if (!valid.id(realmId)) return validE.valid("realmId");
	if (!valid.id(uid)) return validE.valid("uid");
	if (!valid.id(roleIdOrLvl as Id) && !valid.num(roleIdOrLvl as number))
...
export async function realm_user_role_add(
	suser: Socket_User,
	realmId: Id,
	uid: Id,
	roleIdOrLvl: Id | number,
