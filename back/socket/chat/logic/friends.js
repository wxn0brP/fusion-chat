...
export async function friend_request(suser, nameOrId){
    ...
    const friendExists = await db.dataGraph.find("friends", { a: suser._id, b: id });
    ...
    await db.data.add("friendRequests", { from: suser._id, to: id });
...
export async function friend_remove(suser, id){
    ...
    const friendExists = await db.dataGraph.find("friends", suser._id, id);
