...
export type Settings_AllData =
	| Db_RealmConf.category[]
	| Db_RealmConf.channel[]
	| Db_RealmConf.emoji[]
	| Db_RealmConf.webhook[];

// sections that require get db data
const db_data_req_sect = ["categories", "channels", "emojis", "webhooks"];
 ...
 */
export default async function realm_settings_set(
	suser: Socket_User,
	id: Id,
	data: Socket_RealmSettings,
	...
	if (!(await validatePermissions(suser._id, id, data))) {
		return validE.err(
			InternalCode.UserError.Socket
				.RealmSettingsSet_InsufficientPermissions,
		);
	}
	...
	try {
		...
		return validE.err(
			InternalCode.ServerError.Socket.RealmSettingsSet_Failed,
		);
	}
}
