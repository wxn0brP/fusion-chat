...
export default async (id: Id, data: Socket_RealmSettings) => {
	...
	const new_users = new_data
		.filter((user) => /^[a-zA-Z0-9]/.test(user.u))
		.map((user) => {
			return {
				u: user.u,
				r: user.r,
	...
	const changes_users = processDbChanges(
		old_users,
		new_users,
		["u", "r"],
		"u",
	...
	const new_bots = new_data
		.filter((user) => user.u.startsWith("^"))
		.map((user) => {
			return {
				bot: user.u.replace("^", ""),
				r: user.r,
	...
	const changes_bots = processDbChanges(
		old_bots,
		new_bots,
		["bot", "r"],
		"bot",
