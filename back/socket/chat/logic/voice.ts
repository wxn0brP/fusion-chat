...
export async function voice_join(
	socket: Socket,
	to: Id,
...
export async function call_dm_init(
	suser: Socket_User,
	id: Id,
): Promise<Socket_StandardRes> {
	...
	if (sockets.length == 0) {
		const targetName = await db.data
			.findOne<Db_Data.user>("user", { _id: id })
			.then((u) => u.name);
...
export async function call_dm_answer(
	suser: Socket_User,
	id: string,
	answer: boolean,
): Promise<Socket_StandardRes> {
