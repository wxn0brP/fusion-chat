...
export async function message_edit(suser, toM, _id, msg, options:Socket__Mess.message_edit__opts={}){
    options = {
        minMsg: 0,
        maxMsg: 2000,
        ...options,
    ...
    const mess = await db.mess.findOne<Db_Mess.Message>(to, { _id });
    if(!mess){
    ...
    if(privChat){
        ...
        global.sendToSocket(toM.replace("$", ""),  "message.edit", _id, msg, time);
    }else{
        global.sendToChatUsers(toM, "message.edit", _id, msg, time);
    }
...
export async function message_delete(suser, toM, _id){
    ...
    if(privChat){
        ...
        global.sendToSocket(toM.replace("$", ""),  "message.delete", _id);
    }else{
        global.sendToChatUsers(toM, "message.delete", _id);
        const threads = await db.realmData.find<Db_RealmData.thread>(to, { reply: _id });
...
export async function messages_delete(suser, toM, ids){
    ...
    const messages = await db.mess.find<Db_Mess.Message>(to, { $in: { _id: ids } });
    ...
    if(!privChat){
        for(const mess of messages){
            const threads = await db.realmData.find<Db_RealmData.thread>(to, { reply: mess._id });
            for(const thread of threads){
                await realm_thread_delete(suser, toM, thread._id);
            }
        }
    ...
    if(privChat){
        ...
        global.sendToSocket(toM.replace("$", ""),  "messages.delete", ids);
    }else{
        global.sendToChatUsers(toM, "messages.delete", ids);
    }
...
export async function message_markAsRead(suser, to, chnl, mess_id){
    ...
    if(mess_id == "last"){
        ...
        const lastIdMess = await db.mess.find<Db_Mess.Message>(toM, { chnl }, {}, { reverse: true, max: 1 });
...
export async function message_react(suser, realm, msgId, react){
    ...
    const msg = await db.mess.findOne<Db_Mess.Message>(toM, { _id: msgId });
