...
export async function message_edit(
	...
	const dbChatId = isDmChat
		? combineId(suser._id, chatId.replace("$", ""))
		: chatId;

	const mess = await db.mess.c<Db_Mess.Message>(dbChatId).findOne({ _id });
	if (!mess) {
		return validE.err(
			InternalCode.UserError.Socket.MessageEdit_MessageNotFound,
		);
	}
	if (mess.fr !== suser._id) {
		return validE.err(
			InternalCode.UserError.Socket.MessageEdit_NotAuthorized,
		);
	}

	const time = Math.floor(new Date().getTime() / 1000).toString(36);
	await db.mess.c(dbChatId).updateOne({ _id }, { msg, lastEdit: time });
...
export async function message_delete(
	suser: Socket_User,
	chatId: Id,
	_id: Id,
): Promise<Socket_StandardRes> {
	...
	if (!mess) {
		return validE.err(
			InternalCode.UserError.Socket.MessageDelete_MessageNotFound,
		);
	}
	if (mess.fr !== suser._id) {
		const permSys = new permissionSystem(dbChatId);
		const userPerm = await permSys.canUserPerformAction(
			suser._id,
			Permissions.manageMessages,
		);
		if (!userPerm)
			return validE.err(
				InternalCode.UserError.Socket.MessageDelete_NotAuthorized,
			);
	}

	await db.mess.c(dbChatId).removeOne({ _id });
	if (isDmChat) {
		...
		const threads = await db.realmData.c<Db_RealmData.thread>(dbChatId).find({
			reply: _id,
		});
...
export async function messages_delete(
	suser: Socket_User,
	chatId: Id,
	ids: Id[],
): Promise<Socket_StandardRes> {
	...
	const messages = await db.mess.c<Db_Mess.Message>(dbChatId).find({ $in: { _id: ids as any } });
	if (messages.some((mess) => mess.fr !== suser._id)) {
		if (isDmChat)
			return validE.err(
				InternalCode.UserError.Socket.MessagesDelete_NotAuthorized,
			);
		const permSys = new permissionSystem(dbChatId);
		const userPerm = await permSys.canUserPerformAction(
			suser._id,
	...
	await db.mess.c(dbChatId).remove({ $in: { _id: ids } });
	if (isDmChat) {
		...
		const threads = await db.realmData.c<Db_RealmData.thread>(dbChatId).find({ $in: { reply: ids as any }, });
		for (const thread of threads) {
			await realm_thread_delete(suser, dbChatId, thread._id);
		}
	}
...
export async function message_fetch(
	...
	if (!isDmChat) {
		...
		if (!perm.view)
			return validE.err(
				InternalCode.UserError.Socket.MessageFetch_ChannelNotFound,
			);
	}

	const responeAll = await db.mess.c(dbChatId).find(
		{ chnl },
...
export async function message_fetch_id(
	suser: Socket_User,
	chatId: Id,
	chnl: Id,
	mess_id: Id,
	...
	if (!isDmChat) {
		...
		if (!perm.view)
			return validE.err(
				InternalCode.UserError.Socket.MessageFetchId_ChannelNotFound,
			);
	}

	const res = await db.mess.c(dbChatId).findOne({ _id: mess_id });
	return { err: false, res: [res] };
}

export async function message_mark_read(
	suser: Socket_User,
	chatId: Id,
	chnl: Id,
	mess_id: Id,
	...
	if (mess_id == "last") {
		const dbChatId = isDmChat
			? combineId(suser._id, chatId.replace("$", ""))
			: chatId;
		const lastIdMess = await db.mess.c<Db_Mess.Message>(dbChatId).find(
			{ chnl },
	...
	await db.userData.c(suser._id).updateOne(search, {
		$merge: {
			last: {
				[chnl]: mess_id,
			},
...
export async function message_react(
	suser: Socket_User,
	chatId: Id,
	msgId: Id,
	react: string,
	...
	const msg = await db.mess.c<Db_Mess.Message>(dbChatId).findOne({
		_id: msgId,
	});
	if (!msg)
		return validE.err(
			InternalCode.UserError.Socket.MessageReact_MessageNotFound,
		);
	...
	if (!perm.react)
		return validE.err(
			InternalCode.UserError.Socket.MessageReact_NotAuthorized,
		);
	...
	await db.mess.c(dbChatId).updateOne({ _id: msgId }, { reacts });
...
export async function message_search(
	suser: Socket_User,
	chatId: Id,
	chnl: Id,
	query: Socket__Mess.MessageQuery,
): Promise<Socket_StandardRes> {
	...
	const res = await db.mess.c(dbChatId).find(
		(data, context) => {
...
export async function message_pin(
	suser: Socket_User,
	chatId: Id,
	chnl: Id,
	msg_id: Id,
	...
	await db.mess.c(dbChatId).updateOne({ _id: msg_id }, { pinned: pin });
	const refreshData = {
...
export async function message_fetch_pinned(
	suser: Socket_User,
	chatId: Id,
	chnl: Id,
): Promise<Socket_StandardRes> {
	...
	const res = await db.mess.c(dbChatId).find({ chnl, pinned: true });
