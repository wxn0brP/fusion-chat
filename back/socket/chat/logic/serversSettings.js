...
export async function server_settings_get(suser, id, sections=[]){
    const validE = new ValidError("server.settings.get");
    if(!valid.id(id)) return validE.valid("id");
    if(!valid.arrayString(sections)) return validE.valid("sections");
...
export async function server_settings_set(suser, id, data){
    const validE = new ValidError("server.settings.set");
    if(!valid.id(id)) return validE.valid("id");
    ...
    if(!setServerSettingsShema(data)){
        ...
        lo(data)
        return validE.valid("data", setServerSettingsShema.errors);
    }
