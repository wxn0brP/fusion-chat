...
router.get("/get", async (req, res) => {
	...
	const tokenData = (await decode(token, KeyIndex.GENERAL)) as {
		user: string;
	};
	if (!tokenData)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.DeleteAccountGet_InvalidToken,
			msg: "invalid token",
		});
	...
	if (!user)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.DeleteAccountGet_UserNotFound,
...
router.post("/confirm", async (req, res) => {
	...
	if (!tokenData)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.DeleteAccountConfirm_InvalidToken,
			msg: "invalid token",
		});
	...
	if (!user)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.DeleteAccountConfirm_UserNotFound,
			msg: "user is not found",
		});
	...
	if (!isPasswordValid)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express
				.DeleteAccountConfirm_InvalidPassword,
	...
	if (existingTask)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express
				.DeleteAccountConfirm_AlreadyPending,
...
router.post("/undo", async (req, res) => {
	...
	if (!tokenData)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.DeleteAccountUndo_InvalidToken,
			msg: "invalid token",
		});
	...
	if (!task)
		return res.json({
			err: true,
			c: InternalCode.UserError.Express.DeleteAccountUndo_PendingNotFound,
