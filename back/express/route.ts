...
async function renderLayout(res, layout, bodyPath, layoutData, bodyData) {
	res.render(bodyPath, bodyData, (err, body) => {
		if (err) return sendInternalError(res, err);
...
frontRouter.get(
	...
	error500((req, res) => {
		res.render("app/app", (err, body) => {
			if (err) return sendInternalError(res, err);
			const html =
				process.env.NODE_ENV == "development" ? body : minifyHtml(body);
			res.send(html);
		});
	}),
);
...
frontRouter.get(
	"/dev-panel",
	error500((req, res) =>
		renderLayout(res, "layout/main", "dev-panel/dev-panel", {}, {}),
	),
);

readdirSync("front/main", { recursive: true })
	.filter((file) => file.includes(".ejs"))
	.map((file) => (file as string).replace(".ejs", ""))
	.forEach((site) => {
