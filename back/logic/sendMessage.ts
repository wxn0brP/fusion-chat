...
export default async function sendMessage(
	...
	if (privChat) sendDmNotification(originalTo, user, data);
	else await sendReamNotification(originalTo, user, data);
 ...
 */
async function processIdAndPerm(
	req: Request,
	user: User,
	options: Options,
): Promise<Socket_StandardRes> {
	...
	if (privChat) {
		...
		await db.mess.ensureCollection(to);
	} else {
	...
	if (!privChat && !options.system) {
		...
		if (!perm.write)
			return validE.err(
				InternalCode.UserError.Socket.NoPermissionToWriteMessage,
			);
