...
export default class PermissionSystem {
	...
	async validateHierarchy() {
		...
		if (roles.length === 0) return; // System can operate without defined roles
	...
	async createRole(name: string, opts: Logic_PermSys.createRole__opts = {}) {
		...
		if (typeof p !== "number")
			throw new Error(
				"p must be a number or array of permissions bitflags",
			);

		if (managerId) {
			const userHighestRole = await this.getUserHighestRole(managerId);
			if (
				userHighestRole.lvl >= lvl &&
				!this.isUserBeKing(managerId, userHighestRole.lvl)
			)
				throw new Error(
					"Invalid lvl - would break chain with user's highest role",
				);

			const userPerms = await this.getUserPermissions(managerId);
			if (
				!hasAllPermissionsNumber(userPerms, p) &&
				!this.isUserBeKing(managerId, userHighestRole.lvl)
			)
		...
		if (lvl === null) {
			...
			if (insertIndex !== -1) {
				if (insertIndex > 0 && lvl <= roles[insertIndex - 1].lvl)
					throw new Error(
						"Invalid lvl - would break chain with previous role",
					);

				if (insertIndex < roles.length && lvl >= roles[insertIndex].lvl)
					throw new Error(
						"Invalid lvl - would break chain with next role",
					);
			} else {
				lvl = roles.length > 0 ? roles[roles.length - 1].lvl + 1 : 0;
			}
	...
	async updateRole(
		roleId: Id,
		updates: Partial<Db_RealmRoles.role>,
		managerId: Id = null,
	) {
		...
		if (managerId) {
			...
			if (
				!this.hasHigherRole(managerRoles, role.lvl) &&
				!this.isUserBeKing(managerId)
			)
				throw new Error("Insufficient permissions to edit this role");

			if (updates.p !== undefined) {
				const managerPerms =
					this.calculateCombinedPermissions(managerRoles);

				if (
					!canChangePermissions(updates.p, role.p, managerPerms) &&
					!this.isUserBeKing(managerId)
				)
					throw new Error(
	...
	async deleteRole(roleId: Id, managerId: Id | false) {
		...
		if (managerId) {
			...
			if (
				managerHighestRole.lvl >=
				roles.find((r) => r._id === roleId).lvl &&
	...
	async removeRoleFromUser(userId: Id, roleId: Id, managerId: Id) {
		...
		return await this.realmUser.updateOne(
			{ u: userId },
			{ $pull: { r: roleId } },
		);
	}
	...
	calculateCombinedPermissions(roles: Db_RealmRoles.role[]) {
		...
		return roles.reduce(
			(perms, role) => combinePermissions(perms, role.p),
			0,
		);
	}
