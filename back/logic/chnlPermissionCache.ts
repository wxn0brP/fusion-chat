...
export const cache = new AnotherCache<number>(getCacheSettings("ChnlPermission"));
export const channelPermissionsCache = new AnotherCache<Logic_ChnlPerm.ChannelPermissions>(
	getCacheSettings("ChnlPermission_Channels"),
);

const generateCacheKey = (realm: Id, chnl: Id, userId: Id) =>
	`${realm}:${chnl}:${userId}`;
...
async function fetchChannelsPermissions(
	realm: Id,
): Promise<Logic_ChnlPerm.ChannelPermissions> {
	let cachedPermissions =
		channelPermissionsCache.get(realm);
	if (cachedPermissions) return cachedPermissions;

	const channels = await db.realmConf.find<Db_RealmConf.channel>(realm, {
		$exists: { chid: true },
	});
	const permissions = channels.reduce(
		(
			acc: Logic_ChnlPerm.ChannelPermissions,
			channel: Db_RealmConf.channel,
		) => {
...
class PermissionCache {
	...
	async getUserPermissionsInRealm(realm: Id, userId: Id) {
		const keys = cache
			...
			.filter(
				(key) =>
					key.startsWith(`${realm}:`) && key.endsWith(`:${userId}`),
			);
		const permissions = {};
	...
	clearChannelCache(realm: Id, chnl: Id) {
		const keys = cache
			.keys()
			.filter((key) => key.startsWith(`${realm}:${chnl}:`));
		keys.forEach((key) => cache.delete(key));
	}
...
export default async function getChnlPerm(
	...
	const permSys = new PermissionSystem(realm);
	const admin = await permSys.canUserPerformAction(
		user,
		rolePermissions.admin,
	);
	const perms = admin
		? -1
		: await permissionCache.getPermissions(realm, chnl, user);
