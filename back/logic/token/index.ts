...
export async function create(
	...
	encrypt: boolean | KeyIndex = false,
) {
	...
	if (encrypt) {
		const keyIndex =
			typeof encrypt === "number" ? encrypt : KeyIndex.GENERAL;
		...
		if (exp !== false) encryptedToken.setExpirationTime(exp ? exp : "1h");
...
export async function decode(
	...
	try {
		if (keyIndex) {
			...
			const decrypted = (await jwtDecrypt(token, keyPair.privateKey)) as {
