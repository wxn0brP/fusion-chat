...
export const renderWebhooks = function(){
    ...
    if(!settings || !settings.webhooks) return uiFunc.uiMsg(translateFunc.get("No settings data"));

    const container = rs_data.html.webhook;
    container.innerHTML = `<h1>${translateFunc.get("Webhooks")}</h1>`;

    initButton(container, translateFunc.get("Add webhook"), () => {
        ...
        if(!chnl) return uiFunc.uiMsg(translateFunc.get("No channels found"));
...
export const renderWebhookEdit = function(id: Id){
    ...
    if(!webhook) return uiFunc.uiMsg(translateFunc.get("Webhook not found"));
    ...
    addSeparator(container, 15);
    const name = initInputText(container, translateFunc.get("Name"), webhook.name);
    addSeparator(container, 5);
    const template = initInputText(container, translateFunc.get("Template"), webhook.template);
    ...
    const ajv = initInputText(advancedDiv, translateFunc.get("Ajv"), JSON.stringify(webhook.ajv) || "{}");
    addSeparator(advancedDiv, 5);
    const required = initInputText(advancedDiv, translateFunc.get(`Required fields`), JSON.stringify(webhook.required) || "[]");
    ...
    // @ts-ignore
    const embedTitle = initInputText(embedSettings, translateFunc.get("Title"), embed.title || "");
    addSeparator(embedSettings, 5);
    // @ts-ignore
    const embedUrl = initInputText(embedSettings, translateFunc.get("Url"), embed.url || "");
    addSeparator(embedSettings, 5);
    // @ts-ignore
    const embedDesc = initInputText(embedSettings, translateFunc.get("Description"), embed.description || "");
    addSeparator(embedSettings, 5);
    // @ts-ignore
    const embedImage = initInputText(embedSettings, translateFunc.get("Image"), embed.image || "");
    addSeparator(embedSettings, 5);
    ...
    initButton(embedCustomFields, translateFunc.get("Add"), () => createEmbedCustomFields("",""));
    addSeparator(embedSettings, 5);
    ...
    addSeparator(container, 15);
    initButton(container, translateFunc.get("Save"), () => {
    ...
    initButton(container, translateFunc.get("Cancel"), () => {
        container.fadeOut();
        // @ts-ignore
        if(!webhook.embed.title) webhook.embed = undefined;
    });

    initButton(container, translateFunc.get("Delete"), () => {
