...
export default function format_text(text: string) {
    ...
    text = text
        .replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" onclick="app.ui.linkClick(event)">$1</a>')
    ...
    text = text.replace(/:([a-z0-9]+-[a-z0-9]+-[a-z0-9]+):/g, (match, emojiId) => {
        return `<img src="/userFiles/realms/${$store.chat.id.get()}/emojis/${emojiId}.png" class="message_emoji" alt=":${emojiId}:">`;
    });
