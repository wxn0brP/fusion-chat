...
import { Vars_realm__chnlPerm, Vars_realm__thread } from "#types/var";
...
const contextMenu = {
    ...
    message(e: MouseEvent, id: Id, opts: Ui_contextMenu__messageOptions) {
        ...
        const chnlPerm = $store.realm.chnlPerm.get()?.[$store.chat.chnl.get()] || {} as Vars_realm__chnlPerm;
        setDisplayByDataId(ele, "add_reaction", chnlPerm?.react || false);
        setDisplayByDataId(ele, "create_thread", chnlPerm?.threadCreate || false);
    ...
    channel(e: MouseEvent, id: Id, opts: Ui_contextMenu__channelOptions) {
        ...
        setDisplayByDataId(ele, "create_thread", $store.realm.chnlPerm.get()?.[id]?.threadCreate || false);
    ...
    thread(e: MouseEvent, thread: Vars_realm__thread) {
        ...
        const permToDelete = $store.user._id.get() === thread.author || permissionFunc.isAdmin();
...
function canUserManageRealm(id: Id) {
    ...
    const perm = $store.realms.get().find((realm) => realm.realm === id)?.p || 0;
