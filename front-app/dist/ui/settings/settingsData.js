...
import LangPkg, { LangRef, load_translate } from "../../utils/translate.js";
...
const settingsData = {
            ...
            settings: [
                {
                    ...
                    onclick: async () => {
                        ...
                        if (!await uiFunc.confirm(confText))
                            return;
                        if (!await uiFunc.confirm(confText + " (" + LangPkg.settings_user.double_check + ")"))
                            return;
                ...
                },
                {
                    ...
                    type: "button",
                    onclick: async () => {
                        if (!await uiFunc.confirm(LangPkg.settings_user.confirm_delete.w1 + "?"))
                            return;
                        if (!await uiFunc.confirm(LangPkg.settings_user.confirm_delete.w2 + "?"))
                            return;
                        if (!await uiFunc.confirm(LangPkg.settings_user.confirm_delete.w3 + "?"))
                            return;
    ...
    userSave: (settings) => {
        ...
        if (desktopHandling != undefined) {
            ...
            if (apis.app.apiType == "ele")
                apis.api.send({ type: "desktopHandling", data: desktopHandling });
        }
    },
};
