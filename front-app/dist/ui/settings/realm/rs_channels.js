...
import { initInputText, initButton, addSeparator, initCheckbox, } from "./rs_utils.js";
export const renderChannels = function () {
    const rs_data = rs_dataF();
    const settings = rs_data.settings;
    if (!settings || !settings.categories || !settings.channels)
    ...
    sortedCategories.forEach(category => {
        ...
        initButton(categoryDiv, LangPkg.settings_realm.move_down, () => {
            if (category.i === sortedCategories.length - 1)
                return;
            const i = category.i;
            settings.categories[i].i = i + 1;
            settings.categories[i + 1].i = i;
        ...
        initButton(categoryDiv, LangPkg.settings_realm.add_channel, async () => {
            ...
            const type = await uiFunc.selectPrompt(LangPkg.settings_realm.select_type, [text, voice, announcement, open_announcement, forum], ["text", "voice", "announcement", "open_announcement", "forum"]);
            const newChannel = {
        ...
        categoryChannels.forEach(channel => {
            ...
            initButton(channelElement, LangPkg.settings_realm.move_up, () => {
                ...
                const currentChannelIndex = settings.channels.findIndex(ch => {
                    if (ch.category !== channel.category)
                        return false;
                    return ch.i === i;
                });
                ...
                if (currentChannelIndex === -1 || previousChannelIndex === -1)
                    return;
            ...
            initButton(channelElement, LangPkg.settings_realm.move_down, () => {
                if (channel.i >= categoryChannels.length - 1)
                    return;
                const i = channel.i;
                const currentChannelIndex = settings.channels.findIndex(ch => {
                    if (ch.category !== channel.category)
                ...
                if (currentChannelIndex === -1 || nextChannelIndex === -1)
                    return;
                settings.channels[currentChannelIndex].i = i + 1;
...
export const renderEditCategory = function (category) {
    ...
    if (!settings || !settings.categories)
        return debugFunc.msg(LogLevel.ERROR, "No settings data");
    const containerElement = rs_data.html.editChannel;
...
//# sourceMappingURL=rs_channels.js.map