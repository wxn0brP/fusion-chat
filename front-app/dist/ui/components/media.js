...
class MediaPopup {
    ...
    init() {
        ...
        if (isVideo === "auto")
            isVideo = /\.(mp4|webm|ogg|mkv|webm|avi)$/i.test(this.url);
    ...
    setupEventListeners() {
        ...
        document.addEventListener("keydown", (e) => {
            if (e.key === "Escape")
                this.close();
            if (e.key === "+")
                this.zoom(1 + this.options.scaleStep);
            if (e.key === "-")
                this.zoom(1 - this.options.scaleStep);
            if (e.key === "ArrowLeft")
                this.rotate(-this.options.rotationStep);
            if (e.key === "ArrowRight")
                this.rotate(this.options.rotationStep);
        });
    ...
    handleTouchStart(e) {
        ...
        if (touches.length === 1) {
            ...
            if (now - lastTap < this.options.doubleTapDelay &&
                this.getDistance({ x: touch.pageX, y: touch.pageY }, lastTapPosition) < 30) {
    ...
    zoom(scaleFactor, centerX = undefined, centerY = undefined) {
    ...
    getRotationAngle(touches) {
        return Math.atan2(touches[1].pageY - touches[0].pageY, touches[1].pageX - touches[0].pageX) * (180 / Math.PI);
    }
    getDistance(p1, p2) {
