...
const render_realm = {
    ...
    realmUserStatus(id, state) {
        ...
        if (!temp[id]) {
            ...
            temp[id] = {
                status: renderUtils.createUpdater(reRender, status ?? "offline"),
                statusText: renderUtils.createUpdater(reRender, statusText ?? ""),
                activity: renderUtils.createUpdater(reRender, activity ?? null),
            };
        }
        if (status)
            temp[id].status.set(status);
        if (activity)
            temp[id].activity.set(activity);
    }
};
