...
import formatFunc from "../../core/mess/format.js";
import { navHTML, renderHTML } from "../../var/html.js";
import { getChannelTypeEmoticon } from "./realmInit.js";
import uiFunc from "../helpers/uiFunc.js";
const render_events = {
    show() {
        if (vars.chat.to == "main" || vars.chat.to.startsWith("$"))
            return;
        socket.emit("realm.event.list", vars.chat.to, false, (events) => {
    ...
    renderEvent(event) {
        ...
        if (desc) {
            const eventDesc = eventDiv.querySelector("[data-id='eventDesc']");
        ...
        updateCutdown();
        const joinBtn = eventDiv.querySelector("[data-id='users-join']");
        ...
        const usersCount = eventDiv.querySelector("[data-id='users-count']");
        const usersShow = eventDiv.querySelector("[data-id='users-show']");
        const users = eventDiv.querySelector("[data-id='users']");
    ...
    async create() {
        ...
        function renderWhereOptions() {
            navHTML.realm__channels.querySelectorAll(".channel_voice:not(details .channel_voice)").forEach(chnl => {
            ...
            navHTML.realm__channels.querySelectorAll("details").forEach(details => {
                ...
                details.querySelectorAll(".channel_voice").forEach(chnl => {
                    const id = chnl.id.replace("channel_", "");
                    const name = chnl.textContent.replace(getChannelTypeEmoticon("voice") + " | ", "");
                    const option = document.createElement("option");
                    option.value = id;
        ...
        submit.addEventListener("click", () => {
            const req = {
                ...
                desc: desc.value,
                type: type.value,
                where: where.querySelector("select, input").value,
                img: img.value,
            };
