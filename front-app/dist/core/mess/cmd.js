    ...
    system: {
        ...
        clear: {
            ...
            exe(msg, args) {
                ...
                const msgs = Array.from(document.querySelectorAll(".mess_message"))
                    .slice(-args[0])
                    .map(container => {
...
export let currentCmd = null;
export function setCurrentCmd(cmd) {
    currentCmd = cmd;
}
const messCmd = {
    temp: [],
        ...
        Object.keys(messCmds).forEach(category => {
            Object.keys(messCmds[category]).forEach(key => {
                if (key.startsWith(cmdName) || cmdName == "") {
                    if (!availableCmds[category])
                        availableCmds[category] = {};
    ...
    checkArgs() {
        ...
        for (let i = 0; i < argsObj.length; i++) {
            ...
            if (arg.type == "boolean" && val != "true" && val != "false")
                return false;
            else if (arg.type == "number" && isNaN(val))
                return false;
            else if (arg.type == "text" && val.trim() == "")
                return false;
            else if (arg.type == "user" && val.trim() == "")
                return false;
            else if ((arg.type == "date" || arg.type == "date-time") && isNaN(Date.parse(val)))
                return false;
            ...
            else if (arg.type == "list" && !arg.list.includes(val))
                return false;
            else if (arg.type == "map" && Object.keys(val).length == 0)
                return false;
        }
        return true;
    },
